<template>
    <div class="q-date" :class="{'preview-pc': !value.edit}">
        <div class="q-title">
            ...
            <!-- </div> -->
            <c-edit v-if="value.edit" v-model="value.title" :maxLength="50" :canUpload="true" @upload="titleUpload" :imgUrl="value.imgUrl" @uploadFail="titleUploadFail"></c-edit>
        </div>
        <div class="date-wrap" :class="{'preview': !value.edit}" v-if="timeDatepreviewShow">
            <el-date-picker :editable="false" @change="change" type="datetime" v-model="val" @focus="focus" placeholder="请选择日期+时间" :disabled="datetimeDisabled">
            </el-date-picker>
            <span v-if="value.edit" class="flag" @click="switchType(0, formats[0].type, formats[0].format)">{{formats[0].text}}</span>
        </div>
        <div class="date-wrap" :class="{'preview': !value.edit}" v-if="datePreviewShow">
            <!-- 编辑 -->
            <el-date-picker  :editable="false" @change="change" v-model="val" @focus="focus" placeholder="选择日期" :disabled="dateDisabled">
            </el-date-picker>
            <span v-if="value.edit" class="flag" @click="switchType(1, formats[1].type, formats[1].format)">{{formats[1].text}}</span>
        </div>
        <div class="date-wrap" :class="{'preview': !value.edit}" v-if="timePreviewShow">
            <el-time-picker  :editable="false" @change="change" v-model="val" @focus="focus" placeholder="选择时间" :disabled="timeDisabled">
            </el-time-picker>
            <span v-if="value.edit" class="flag" @click="switchType(2, formats[2].type, formats[2].format)">{{formats[2].text}}</span>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            titleUpload(src) {
                this.errorTip = ''
                mutations.getDateTitleUrl(src, this.index)
            },
            titleUploadFail(msg) {
            ...
            switchType(index, type, format) {
                ...
                this.curType = type
                this.curIndex = index
                mutations.getDateFormat(this.formats[index]._format, this.index)
            },
            change(val) {
                console.log(val)
                let reply = this.format(this.curType, val)
                mutations.getDateReplys(reply, this.index)
                // console.log(reply)
                // console.log(this.format(this.curType, val))
            },
            focus() {
                this.errorTip = ''
            },
