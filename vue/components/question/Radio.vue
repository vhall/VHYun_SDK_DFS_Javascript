<template>
  <div class="q-radio" :class="{ 'preview-pc': !value.edit }">
    ...
    <div class="items">
      ...
      <div v-if="value.edit" class="add-items" @click="addItems(false)">
...
export default {
  ...
  methods: {
    addItems(isOther = false) {
      mutations.addItems(this.index,isOther);
    },
    change(idx) {
      mutations.getRadioAnswer(idx, this.index);
    },
    changeReplys(key) {
      mutations.getRaidoReplys(key, this.index);
    },
    delItem(idx) {
      // this.val = ''
    ...
    titleUpload(data) {
      mutations.getRadioTitleImgUrl(data, this.index);
    },
    titleUploadFail(msg) {
    ...
    answerUpload(src, idx) {
      this.errorTip = "";
      this.$forceUpdate();
      mutations.getRadioItemsUrl(src, idx, this.index);
    },
