  ...
  <div class="q-matrix-wrap" :class="{ 'preview-pc': !value.edit }">
      ...
      <div class="row-title">
        ...
        >
          <div class="item-row">
            ...
            >
              <i class="iconfont icon-more-line-copy"></i>
      ...
      <div class="main-table" ref="main-table">
        ...
        <div class="addCol" v-if="value.edit">
          <span @click="addCol(value.column)">
            <i class="iconfont icon-chuangjian"></i>添加列
          </span>
        </div>
      </div>
    </div>
    <div class="addRow" v-if="value.edit">
      <span @click="addRow(value.row)">
        <i class="iconfont icon-chuangjian"></i>添加行
      </span>
    </div>
...
export default {
  ...
  methods: {
    upload(src) {
      this.errorTip = "";
      mutations.getMatrixTitleUrl(src, this.index);
    },
    uploadFail(msg) {
      this.errorTip = msg;
    },
    addRow(e) {
      mutations.addMatrixRow(this.index, e.length + 1);
    },
    addCol(e) {
      mutations.addMatrixCol(this.index, e.length + 1);
    },
    check() {
      if (
        this.value.required &&
    ...
    del(selectIndex, type) {
      // mutations.del(index, type, this.index);
      let format = this.value.format;
      if (type === "row") {
    ...
    changeCorrect(format) {
      ...
      if (this.value.edit) {
        mutations.getMartixAnswer([...answerArr], this.index);
      } else {
        mutations.getMartixReplys([...answerArr], this.index);
      }
      this.$forceUpdate();
    },
