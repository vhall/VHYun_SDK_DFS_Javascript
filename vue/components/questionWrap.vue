    ...
    >
      ...
      <div class="q-operate" v-if="isShowOperate && value.edit">
        ...
        <div class="right">
          ...
          ></el-checkbox>
          <i
            class="elicon el-icon-document-add"
            title="创建题目"
          ...
          <i
            ...
            title="复制题"
          ></i>
          <div class="del icon" @click.stop="remove">
            <i class="iconfont icon-shanchu"></i>
          </div>
          <div class="sort icon" @click.stop="sort">
            <i class="iconfont icon-yidong-moren"></i>
          </div>
        </div>
      </div>
    </div>
...
<script>
import { types as QTypes, coms as QComs } from "../common/contant";
...
export default {
  ...
  methods: {
    ...
    updateQuestion() {
      const { id = "" } = this.value;
      if (!id) {
        this.$message.error("该题未创建");
        return;
      }
      const option = {
        id, // 题目id
        detail: this.value // 实体类
      };
    ...
    merge() {
      this.$emit("merge", this.index);
    },
    copyPage(pageId) {
      this.$emit("copyPage", pageId);
    },
    delPage(pageId) {
